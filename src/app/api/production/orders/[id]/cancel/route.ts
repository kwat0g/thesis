...
export async function POST(
  ...
  try {
    ...
    await requirePermission(user, 'PROD.CANCEL_ORDER');
    ...
    const success = await productionOrderService.cancelProductionOrder(id, user.userId, reason);
    ...
    return successResponse(productionOrder, 'Production order cancelled successfully');
  } catch (error: any) {
    ...
    return serverErrorResponse(error.message || 'Failed to cancel production order');
  }
}
