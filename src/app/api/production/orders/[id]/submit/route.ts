...
export async function POST(
  ...
  try {
    ...
    const success = await productionOrderService.submitForApproval(id, user.userId);
    ...
    return successResponse(productionOrder, 'Production order submitted for approval');
  } catch (error: any) {
    ...
    console.error('Submit production order error:', error);
    return serverErrorResponse(error.message || 'Failed to submit production order');
  }
}
