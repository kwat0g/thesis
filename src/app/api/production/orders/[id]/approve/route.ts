...
export async function POST(
  ...
  try {
    ...
    const success = await productionOrderService.approveProductionOrder(id, user.userId);
    ...
    return successResponse(productionOrder, 'Production order approved successfully');
  } catch (error: any) {
    ...
    return serverErrorResponse(error.message || 'Failed to approve production order');
  }
}
