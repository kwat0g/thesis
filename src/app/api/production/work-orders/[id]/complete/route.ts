...
export async function POST(
  ...
  try {
    ...
    const success = await workOrderService.completeWorkOrder(id, user.userId);
    ...
    return successResponse(wo, 'Work order completed');
  } catch (error: any) {
    ...
    return serverErrorResponse(error.message || 'Failed to complete work order');
  }
}
