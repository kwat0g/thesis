...
export async function POST(
  ...
  try {
    ...
    const success = await workOrderService.startWorkOrder(id, user.userId);
    ...
    return successResponse(wo, 'Work order started');
  } catch (error: any) {
    ...
    if (error.message.includes('Only released')) {
      return errorResponse(error.message, 400);
    }
    console.error('Start work order error:', error);
    return serverErrorResponse(error.message || 'Failed to start work order');
  }
}
