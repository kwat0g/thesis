...
export async function POST(
  ...
  try {
    ...
    const success = await purchaseOrderService.submitPOForApproval(id, user.userId);
    ...
    return successResponse(po, 'Purchase order submitted for approval');
  } catch (error: any) {
    ...
    return serverErrorResponse(error.message || 'Failed to submit purchase order');
  }
}
